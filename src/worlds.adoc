...
There are four levels of Worlds support on RISC-V harts.  The first level
ISA extension, *Smwid*, defines the WID for all privilege
modes on a hart. This is lockable and may already be locked and read-only out
of reset.  The second level is the *Smlwid* extension, which
enables M-mode to control the WID of lower-privilege modes.  The third
level is the *Smwidlist* extension, which allows the root of trust to constrain
the WIDs that may be used at lower privilege levels (than M).
The fourth
...
The RISC-V Worlds Smwid, Smlwid, Smwidlist, Smwdeleg, and Sswid extensions
allow a hart to assign WIDs to its privilege modes.  The new CSRs are listed in
the Table below.
...
| `XLEN` | `mwid` | WARL for M until locked | ??? | WID used for M-mode.
| `XLEN` | `mlwid` | WARL for M | `0x390` | WID used for lower privilege modes.
`Ceil(Log~2~NWorlds)` LSBs are used, others are zero.
| `XLEN` | `mwidlist` | WARL for M until locked | ??? | Set of WID values
delegated to M-mode, represented as a bit vector. `NWorlds` LSBs are used,
others are zero.
...
| `XLEN` | `slwid` | WARL for [H]S | `0x190` | WID value used in lower modes
...
In this case, the hart is reset into a single world and all transactions from
that hart, regardless of privilege mode, are tagged with the WID of that world.
The `mwid` register is lockable until reset, and on some systems it may already
be set and locked when the hart comes out of reset.

NOTE: The reset value of `mwid` may be set via pinstrap, fuse, or another
control mechanism external to the hart. These considerations are
platform-specific.
...
The Smwidlist extension adds the `mwidlist` CSR, which is locked whenever
`mwid` is.  `mwidlist` (width of NWorlds) represents a set of WIDs as a bit
vector, with WID _i_ represented by bit _i_ of the register.  Transactions
issued by privilege modes S and U use the WID in `mlwid`. The bit in `mwidlist`
that corresponds to `mlwid` must be set to 1 for the transaction to be emitted,
otherwise any load/store/fetch transaction for that privilege mode will trap
with a software check exception. Normally, this situation would be observed
immediately after an MRET or SRET instruction, as the first instruction fetch
in the resulting privilege mode will fail. However, M-mode might instead
attempt to perform a load or store operations "as" that inferior privilege mode
using MPRV, in which case the software check exception will be caused by the
resulting load or store operation.

M-mode may have a different WID than any assignable by `mlwid` to
lower-privilege modes, i.e. `mwid` might not be asserted in `mwidlist`.

NOTE: The platform is not required to allow lower-privilege modes to
be in the same world as M-mode. (The bit in `mwidlist` corresponding to
`mwid`'s value may be zero.)
...
unauthorized WID is written to `slwid`, the lowest-numbered legal WID is
